def MinValue(node, path=[]):
    ...
    for child in node.children:
        temp, child_path = MaxValue(child, path + [node])
...
def MaxValue(node, path=[]):
    ...
    for child in node.children:
        temp, child_path = MinValue(child, path + [node])
        if temp.value > node.value:
            node.value = temp.value
            best_path = child_path
